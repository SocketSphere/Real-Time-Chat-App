...
export const sendMessage = async (req, res) => {
  try {
    ...
    // In sendMessage function, after saving message:

// Broadcast group messages via WebSocket
if (group) {
  console.log('🔌 Broadcasting to group:', group);
  ...
  for (const memberId of memberIds) {
    if (memberId !== sender) {
      const sent = webSocketManager.sendMessageToUser(memberId, {
        type: 'new_message',
        data: {
      ...
      if (sent) {
        console.log(`✅ Sent to group member ${memberId}`);
      } else {
        console.log(`❌ Failed to send to group member ${memberId}`);
      }
    }
  }
  
  // Also send to sender for confirmation
  webSocketManager.sendMessageToUser(sender, {
    type: 'message_sent',
    data: {
      _id: message._id,
      status: 'delivered'
